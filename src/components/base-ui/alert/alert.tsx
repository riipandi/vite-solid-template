...
export const alertVariants = cva(
  'relative w-full rounded-lg border px-4 py-3 text-sm [&:has(svg)]:pl-11 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:top-4 [&>svg]:left-4 [&>svg]:text-foreground',
  {
    variants: {
      variant: {
...
export const AlertTitle = (props: ComponentProps<'div'>) => {
  ...
  return <div class={clx('font-medium leading-5 tracking-tight', local.class)} {...rest} />
}

export const AlertDescription = (props: ComponentProps<'div'>) => {
  const [local, rest] = splitProps(props, ['class'])

  return <div class={clx('text-sm [&_p]:leading-relaxed', local.class)} {...rest} />
}
